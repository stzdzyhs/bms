...
<tr>
  <td class="leftWrap">
    <div class="logo_info"><a href="${path}/main.action"><img src="${path}/images/logo.png" alt="db" title="db" /></a></div>
    <div class="bannerTitle">BMS</div>
    <div class="user_info">
      <p> 欢迎您，<strong>${activeOperator.operatorName }</strong></p>
      <br />
      <p>[<a href="javascript:;" onClick="changePassword('${path}/toChangePassword.action');">基本资料</a> | <a href="javascript:;" onClick="logoutOpen('${path}/logout.action');">退出</a>]</p>
    </div>
...
<%--           <li class="showMenu" id="menu_0"><img src="${path}/images/monitor.png" />调度监控
          ...
          <c:forEach var="u"  items="${menuList}" > 
          	<c:forEach items="${activeList}" var="r">
			            ...
			            <c:forEach items="${u.commands}" var="u1">
				            <c:forEach items="${activeList}" var="r1">
					            <c:if test="${r1.commandNo == u1.commandNo}">
					            	<c:if test="${u1.commandLevel == 2 && u1.isMulti==1}">
						              <li class="subMenu" id="menu_${u1.commandId }"><span>${u1.commandName }</span></li>
						              <script type="text/javascript">
						              	var menu_array_${u1.commandId } = new Array();<c:forEach items="${u1.commands}" var="u2"><c:forEach items="${activeList}" var="r2"><c:if test="${r2.commandNo == u2.commandNo}">
						              	menu_array_${u1.commandId }.push({id:"${u2.commandId }", name:"${u2.commandName }", url:"${path}${u2.moduleName }"});</c:if></c:forEach></c:forEach>
						              </script>
					           	   </c:if>
					           	   <c:if test="${u1.commandLevel == 2 && u1.isMulti==0}">
					           	       <li class="subMenu" id="menu_${u1.commandId }"><span>${u1.commandName }</span></li>
					           	      <script type="text/javascript">
					           	       var menu_array_${u1.commandId } = new Array();
					           	       menu_array_${u1.commandId }.push({id:"${u1.commandId }", name:"${u1.commandName }", url:"${path}${u1.moduleName }"});
					           	    </script>
					           	    </c:if>
					     	   </c:if>
				    	   </c:forEach>
				...
				<script type="text/javascript">
				var menu_array_${u.commandId } = new Array();<c:forEach items="${u.commands}" var="u1"><c:forEach items="${activeList}" var="r1"><c:if test="${u1.commandLevel == 3 && r1.commandNo == u1.commandNo}">
				menu_array_${u.commandId }.push({id:"${u1.commandId }", name:"${u1.commandName }", url:"${path}${u1.moduleName }"});</c:if></c:forEach></c:forEach>
				</script>
          		</c:if>
         	</c:if>
        </c:forEach>
  ...
  <td class="rightWrap">
    ...
    <div id="main_content" style="min-width: 1050px;">
      <iframe id="mainFrame" name="mainFrame" style="display:block" frameborder="0" scrolling="no" src="${path}/views/welcome.jsp"></iframe>
    </div>
  </td>
</tr>
</table>
...
<div id="about">
  <div class="close"><!-- a title="关闭" onClick="$('#about').slideUp('slow');" href="javascript:void(0)">×</a> -->
    <a rel="nofollow" title="版本信息" href="javascript:void(0);" onClick="infoOpen('${path}/aboutInfo.action');" class="version">版本信息</a>

...
function deleteSets(id) {
	...
	for (var i=0; i<sets.length; i++){
		v = $("#operatorStatus" + sets[i].value).val();
		if(v==OPERATOR_STATUS_ENABLED) {
			art.dialog.alert("不能删除启用的用户");
			return false;
		}	
	}
	...
	art.dialog.confirm('你确认删除操作？', function(){
			var options = {
				url: "${path}/sysmgmt/user/userDelete.action?rtId="+id,
				dataType: 'html',
				beforeSend: function() {
					art.dialog.through({
						id: 'broadcastLoading',
...
function goBack(){
	document.getElementById("form1").action = "${path}/sysmgmt/user/userList.action";
...
function toEditPage(id){
	...
	if (type == 2){
		art.dialog.alert("您没有添加或编辑用户的权限！");
		return false;
	}
	...
	$("#id").val(id);
	document.getElementById("form1").action = "${path}/sysmgmt/user/userEdit.action";
	document.getElementById("form1").submit();
}
...
<form action="${path}/sysmgmt/user/userList.action" method="post" name="form1" id="form1">
      ...
      <tbody>
		...
		<tr>
      	...
      	<c:forEach items="${list}" var="u">
   		<input type="hidden" disabled="disabled" id="operatorStatus${u.operatorNo}" value="${u.status}"/>
        <tr>
          	<td align="center"><input type="checkbox" name="rtId" value="${u.operatorNo }">
          	</td>
     		...
     		<c:if test="${u.totalSize == -1 }">
          	...
          	<td class="tdOpera2" align="center">
             	<c:if test="${detail}"><a href="javascript:;" onclick="detail('${u.operatorNo }')">详情</a></c:if>
             	<c:if test="${edit}"><a href="javascript:;" onclick="toEditPage('${u.operatorNo }')">编辑</a></c:if>
             	<c:if test="${delete}"> <a href="javascript:;" onclick="deleteSets('${u.operatorNo }')">删除</a></c:if>
          	</td>
        </tr>
        </c:forEach>
      </tbody>

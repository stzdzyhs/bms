...
function saveUser(){
	...
	var cmsSets = $("input[name='cmd']:checked");
	if(cmsSets.length > 0){
		var tempCmd = "";
		for ( var i = 0; i < cmsSets.length; i++) {
			if(i == 0){
				tempCmd = cmsSets[i].value;
			}
			else{
				tempCmd = tempCmd +","+cmsSets[i].value;
			}
		}
		document.getElementById("cmdStr").value = tempCmd;
	}
	
	art.dialog.confirm('您确认该操作吗？', function(){
			var options = {
				url: "${path}/sysmgmt/user/saveResourceDistOper.action",
				dataType: 'html',
				beforeSend: function() {
					art.dialog.through({
						id: 'broadcastLoading',
...
function goBack(){
	document.getElementById("form1").action = "${path}/sysmgmt/user/resourceOperNoAllocList.action";
...
<form action="${path}/sysmgmt/user/resourceOperNoAllocList.action" method="post" name="form1" id="form1">
...
<input type="hidden" name="cmdStr" id="cmdStr" value="${cmdStr}" />
<fieldset><legend class="legend2">&nbsp;权限设置</legend>
<div id="options" class="searchWrap allowWrap">
	<table class="tableCommon tdBorWhite" width="100%" border="0"cellspacing="0" cellpadding="0">
	  <tr>
	    <td class="tdBlue" >
	    	<c:if test="${type==1}">
    			<input type="checkbox" class="checkbox" id="allowRelation" name="cmd" value="1" />允许关联相册
		    	&nbsp;&nbsp;&nbsp;&nbsp;
    			<input type="checkbox" class="checkbox" id="allowRelease" name="cmd" value="2" />允许发布相册
	    	</c:if>
	    	<c:if test="${type==2}">
	    		<input type="checkbox" class="checkbox" id="allowAddPic" name="cmd" value="3" />允许新增图片
		    	&nbsp;&nbsp;&nbsp;&nbsp;
	    	</c:if>
	    	<c:if test="${type==4}">
	    		<input type="checkbox" class="checkbox" id="allowAddSubColumn" name="cmd" value="6"  />允许添加关联子版块
		    	&nbsp;&nbsp;&nbsp;&nbsp;
		    	<input type="checkbox" class="checkbox" id="allowAddArticle" name="cmd" value="7" />允许添加文章
	    	</c:if>
	    </td>
	  </tr>
	</table>

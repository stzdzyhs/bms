...
function publish() {
	...
	var columnNo = $("#parentId").val();
	if (columnNo == ''){
		art.dialog.alert("请选择所属版块！");
		return false;
	}
	art.dialog.confirm('你确认发布操作？', function(){
			var options = {
				url: "${path}/textmgmt/article/articlePublish.action",
				dataType: 'html',
				beforeSend: function() {
					art.dialog.through({
						id: 'broadcastLoading',
...
<input type="hidden" name="articleNos" id="articleNos" value="${articleNos}" />
<table class="tableCommon tdBorWhite " width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr id="topicTr">
    <td width="35%" class="tdBlue2">所属版块</td>
    <td class="tdBlue">
      <select id="parentId" name="parentId" class="form130px"  >
                  		<option value="">--请选择--</option>
					  	<c:forEach items="${columnList}" var="c" > 
							<option value="${c.columnNo }">${c.columnName }</option>
				 		</c:forEach> 
	</select>
	</td>
  </tr>

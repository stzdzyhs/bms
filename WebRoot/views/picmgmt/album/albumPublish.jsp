...
$(document).ready(function(){
 	...
 	$("input[name='parentType']").change(function() {
		...
		if (parentType == ENTITY_TYPE_MENU) {
			$("#topicTr").hide();
			$("#columnTr").show();
		} else {
			$("#topicTr").show();
...
<table class="tableCommon tdBorWhite " width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td class="tdBlue">
	    <input type="radio" value="1" name="parentType" checked>专题&nbsp;&nbsp; <!-- EntityType.TYPE_TOPIC -->
	    <input type="radio" value="2" name="parentType">栏目<!-- EntityType.TYPE_MENU -->
    </td>
  </tr>
  <tr id="topicTr">
...
<tr>
   <td class="tdBlue2">策略</td>
	<td class="tdBlue">
		...
		<fieldset style="width: 75%" id="strategyFSet">
			<legend id="num_showStrategys">
				<c:if test="${publish.strategys == null }">
					已选0个
				</c:if>
				<c:if test="${publish.strategys != null }">
					已选${fn:length(publish.strategys) }个
				</c:if>
			</legend>
			<div class="checkboxitems" id="showStrategys" style="display: none">
				<c:forEach items="${publish.strategys}" var="code">
					<li>
					  <label> 
					    <input type="checkbox" class="checkbox" checked value="${code.strategyVal }" name="strategyIds" id="strategyIds" /> ${code.strategyVal} 
					 </label>
				   </li>
			   </c:forEach>
			</div>

...
function deleteSets(id) {
	...
	var url="${path}/opermgmt/company/companyDelete.action";
	if (id == '') {
		...
		url="${path}/opermgmt/company/companyDelete.action?rtId="+id;
	...
	art.dialog.confirm('你确认删除操作？', function(){
				...
				success: function(data) {
					...
					if(rsobj.result=="true"){
						...
						if (rsobj.desc == 'operator') {
							art.dialog.alert("该运营商或者所包含的下属运营商被操作员引用，不能删除！");
						}else if (rsobj.desc == 'role') {
							art.dialog.alert("该运营商或者所包含的下属运营商被角色引用，不能删除！");
						}else if(rsobj.desc == 'child'){
							art.dialog.alert("该运营商包含下属运营商，不能删除！");
						}else if(rsobj.desc == 'album'){
							art.dialog.alert("该运营商被相册引用，不能删除！");
						}else if(rsobj.desc == 'topic'){
							art.dialog.alert("该运营商被专题引用，不能删除！");
						}else if(rsobj.desc == 'column'){
							art.dialog.alert("该运营商被版块引用，不能删除！");
						}else if(rsobj.desc == 'article'){
							art.dialog.alert("该运营商被文章引用，不能删除！");
						}else if(rsObj.desc == 'reference'){
							art.dialog.alert("该运营商被策略引用，不能删除！");
						}else {
							art.dialog.alert("删除失败！");
						}
					}
			...
			jQuery('#companyForm').ajaxSubmit(options);
		}, function(){
...
function goBack(){
	document.getElementById("companyForm").action = "${path}/opermgmt/company/companyList.action";
...
function toEditPage(id){
	...
	$("#id").val(id);
	document.getElementById("companyForm").action = "${path}/opermgmt/company/companyEdit.action";
	document.getElementById("companyForm").submit();
}

function detail(id){

	art.dialog.open("${path}/opermgmt/company/companyDetail.action?companyNo=" + id, 
			{
				title: "运营商详情", 
				width: 450,
				height: 400,
...
function regionMgmt(companyNo){
	art.dialog.open("${path}/opermgmt/company/companyCardRegionSelect.action?companyNo=" + companyNo, 
			{
				title: "区域管理", 
				width: 980,
				height: 450,
...
<form action="${path}/opermgmt/company/companyList.action" method="post" name="companyForm" id="companyForm">
              ...
              <tbody>
                ...
                <td width="90px" height="30">所属运营商：</td>
                  ...
                  <td width="160">
                  	...
                  	onMouseOver="toolTip('运营商名称由中文、大小写英文字母、数字、以及下划线组成')" onMouseOut="toolTip()" autocomplete="off" oncontextmenu="return false;"/>
        ...
        <tr>
          <td align="center">
	          <c:if test="${u.companyNo != null && u.companyNo != -1}">
	          	<input type="checkbox" name="rtId" value="${u.companyNo }">
	          </c:if>
          </td>
          <td align="center">${u.companyId }</td>
          <td align="center">${u.companyName }</td>
          <td align="center">
          ...
          <c:if test="${detail}"><a href='javascript:;' onclick='detail(${u.companyNo})'>详情</a></c:if>
          <c:if test="${edit && activeOperator.type !=2}"><a href="javascript:;" onclick="toEditPage('${u.companyNo }')">编辑</a></c:if>
          <!-- comment out by MiaoJun on Apri, 23 2016. cancel the relationship between company with region 
          <c:if test="${region && u.parentId != -1}"> <a href="javascript:;" onclick="regionMgmt('${u.companyNo}')">区域管理</a></c:if>
          -->
          <c:if test="${delete}">
             <c:if test="${u.parentId != -1 && activeOperator.type !=2}">
                <a href="javascript:;" onclick="deleteSets('${u.companyNo }')">删除</a>
             </c:if>
         </c:if>
          </td>
        </tr>

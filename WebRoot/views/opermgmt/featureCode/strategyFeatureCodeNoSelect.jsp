...
function saveResource(){
	...
	sets.each(function() {
		var id = $(this).val();
		var d = {featureCodeNo:id, featureCodeDesc: $("#name"+id).text() };
		data.push(d);
	});
...
function detail(no){
	art.dialog.open("${path}/opermgmt/featureCode/featureCodeDetail.action?featureCodeNo=" + no, 
	{
		title: "特征码详情", 
		width: 450,
		height: 450,
...
<form action="${path}/opermgmt/strategy/strategyFeatureCodeNoSelect.action" method="post" name="form1" id="form1">
...
<div class="searchWrap">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tbody>
        	<tr>
            	<td width="90px" height="30" align="right">描述：</td>
                <td width="160">
      				<input name="featureCodeDesc" class="form130px" value="${search.featureCodeDesc}"  maxlength="50"/>
				</td>
	...
	<table class="tableCommon tableInterlace" width="100%" border="0" cellspacing="0" cellpadding="0">
		<thead>
      	...
      	<tbody>
        	...
        	<tr>
          		<td align="center"><input type="checkbox" name="resourceIds" value="${r.featureCodeNo}">
          		</td>
          		<td align="center" id='name${r.featureCodeNo}'>${r.featureCodeDesc}</td>
          		
          		<td align="center"> 
	          		<c:forEach items="${featureCodeMap}" var="m" > 
						<c:if test="${r.featureCodeType==m.key }"> ${m.value}</c:if>
			    	</c:forEach>
		    	</td>

          		<td align="center">${r.featureCodeVal}</td>
          		<td align="center">${r.createTime}</td>
          		<td class="tdOpera2" align="center">
             		<a href="javascript:;" onclick="detail('${r.featureCodeNo}')">详情</a>
          		</td>
        	</tr>
    		</c:forEach>
		</tbody>

...
function selectFiles(){
	...
	var filePaths = $("input[name='filePaths']:checked");
	...
	for (var i=0; i < filePaths.length; i++ ){
		var filePath = ${ftpServerId} + workDirectory + filePaths[i].value;
		filePathList += "<p>" + "<input type='hidden' name='filePaths' value='" + filePath + "' />" + workDirectory + filePaths[i].value + "</p>";
	}
	
	var fileListTr = artDialog.open.origin.artDialog.open.origin.document.getElementById('fileListTr');
	$(fileListTr).show();
	var fileHtml = artDialog.open.origin.artDialog.open.origin.document.getElementById('fileList').innerHTML + filePathList;
	artDialog.open.origin.artDialog.open.origin.document.getElementById('fileList').innerHTML = fileHtml;
...
function toBrowse(path, name){
	...
	document.getElementById("ftpServerForm").action = "${path}/opermgmt/ftp/selectFtpFileList.action?first=true";
...
function previousDir(){
	...
	document.getElementById("ftpServerForm").action = "${path}/opermgmt/ftp/selectFtpFileList.action";
	document.getElementById("ftpServerForm").submit();
}
...
<form action="${path}/opermgmt/ftp/selectFtpFileList.action" method="post" name="ftpServerForm" id="ftpServerForm">
...
<input type="hidden" name="workDirectory" id="workDirectory_id" value="${workDirectory}" />
  ...
  <div class="tableWrap">
    <table class="tableCommon tableInterlace" width="100%" border="0" cellspacing="0" cellpadding="0">
      <thead>
        <tr>
          <th width="5px"><input type="checkbox" id="pDel" onclick="checkAll(this, 'filePaths')"></th>
          <th width="*%">名称</th>
          <th width="*%">类型</th>
      ...
      <c:if test="${subFileList == null || fn:length(subFileList) == 0}">
        ...
        <tr>
           <td align="center"> <c:if test="${f.type == 0 }"><input type="checkbox" name="filePaths" value="${f.name}"></c:if></td>
...
<%--           <c:if test="${f.type == 0}">
            <a href="javascript:;" onclick="selectFiles('${f.name}','${f.size}')">选择</a>
          </c:if> --%>
          <c:if test="${f.type == 1}">

...
$(document).ready(function(){
	$("#name").keyup(function(){
		...
		if (name != null && name != '' && !resNameReg.test(name)){	
...
function toSelect(fileName,fileSize){
	...
	artDialog.open.origin.artDialog.open.origin.document.getElementById('videoName_id').value = name;
	artDialog.open.origin.artDialog.open.origin.document.getElementById('videoFileUrl_id').value = '${videoFileUrl}' + fileName;
	artDialog.open.origin.artDialog.open.origin.document.getElementById('filesize_id').value = fileSize;
...
function toBrowse(path, name){
	...
	document.getElementById("ftpServerForm").action = "${path}/opermgmt/ftp/browseFtpServer.action?first=true";
...
function previousDir(){
	...
	document.getElementById("ftpServerForm").action = "${path}/opermgmt/ftp/browseFtpServer.action";
	document.getElementById("ftpServerForm").submit();
}
...
<form action="${path}/opermgmt/ftp/browseFtpServer.action" method="post" name="ftpServerForm" id="ftpServerForm">
          ...
          <td class="searchBg"><table class="search" id="searchb" width="100%" border="0" cellspacing="0" cellpadding="0">
              <tbody>
                <tr>
                  ...
                  <td width="160">
                     <input id="name" name="name" class="form130px" maxlength="30" value="<c:out value="${search.name}"/>"  autocomplete="off" oncontextmenu="return false;"/>
                  </td>
      ...
      <c:if test="${subFileList == null || fn:length(subFileList) == 0}">
        ...
        <tr>
         ...
         <c:if test="${f.type == 1 }">
          ...
          <c:if test="${f.type == 0 and select == true}">

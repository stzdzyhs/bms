	...
	if (id == '') {
		...
		for (var i=0; i<sets.length; i++){
			...
			if (activeOpNo != operatorNo){
				art.dialog.alert("您没有权限删除其他人添加的CA卡！");
				return false;
			}
		}
	} else {
...
function toEditPage(id){
	...
	if (type == 2){
		...
		if (typeof(id)!='undefined' && activeOpNo != operatorNo){
			art.dialog.alert("您没有权限编辑其他人添加的CA卡！");
			return false;
		}
	}
  ...
  <div class="tableWrap">
    <table class="tableCommon tableInterlace" width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <c:forEach items="${list}" var="u">
      	<input type="hidden" name="oldOperatorNo${u.clientNo}" id="oldOperatorNo${u.clientNo}" value="${u.operator.operatorNo}" />
        <tr>
          <td align="center">
          ...
          <c:if test="${detail}"><a href='javascript:;' onclick='detail(${u.clientNo})'>详情</a></c:if>
          <c:if test="${edit}"><a href="javascript:;" onclick="toEditPage('${u.clientNo }')">编辑</a></c:if>
          <!-- 
          <c:if test="${region && u.parentId != -1}"> <a href="javascript:;" onclick="regionMgmt('${u.clientNo}')">区域管理</a></c:if>
           -->
          <c:if test="${delete}">
                <a href="javascript:;" onclick="deleteSets('${u.clientNo }')">删除</a>
         </c:if>
          </td>
        </tr>
      </c:forEach>

...
$(document).ready(function(){
	$.formValidator.initConfig({mode:"AutoTip",formID:"form1",ajaxForm:{
			...
			dataType : "html",
			buttons:$("#button"),
			url: "${path}/opermgmt/featureCodeGroup/saveOrUpdate.action",
			beforeSend: function() {
				art.dialog.through({
					id: 'broadcastLoading',
					title: false,
	...
	$("#groupId").formValidator({onFocus:"请输入中文、英文、数字及下划线",onCorrect:"&nbsp;"})
	.regexValidator({regExp:"resname",dataType:"enum",onError:"特征码组ID格式不正确，请确认"})
	.ajaxValidator({
    type : "POST",
	dataType : "html",
	async : true,
	url : "${path}/opermgmt/featureCodeGroup/checkGroupId.action",
	success : function(data){
		eval("var rsobj = "+data+";");
		if(rsobj.result=="true"){
			return true;
	...
	onError : "该特征码组ID已存在",
	onWait : "正在对特征码组ID进行合法性校验，请稍候..."
	}).defaultPassed();
	
	$("#groupName").formValidator({onFocus:"请输入中文、英文、数字及下划线",onCorrect:"&nbsp;"})
		.regexValidator({regExp:"resname",dataType:"enum",onError:"特征码组名称格式不正确，请确认"})
    	.ajaxValidator({
	    type : "POST",
		dataType : "html",
		async : true,
		url : "${path}/opermgmt/featureCodeGroup/checkGroupName.action",
		success : function(data){
			eval("var rsobj = "+data+";");
			if(rsobj.result=="true"){
				return true;
		...
		error: function(jqXHR, textStatus, errorThrown){art.dialog.alert("服务器没有返回数据，可能服务器忙，请重试"+errorThrown);},
		onError : "该特征码组名称已存在",
		onWait : "正在对特征码组名称进行合法性校验，请稍候..."
	}).defaultPassed();

});

function goBack(){
	document.getElementById("form2").action = "${path}/opermgmt/featureCodeGroup/featureCodeGroupList.action";
...
<input type="hidden" name="operatorNo" id="operatorNo" value="${group.operatorNo }" />
<input type="hidden" name="createTime" id="createTime" value="${group.createTime }" /> 
<table class="tableCommon tdBorWhite " width="100%" border="0" cellspacing="0" cellpadding="0">
	
  <tr>
    <td width="35%" class="tdBlue2">特征码组Id</td>
    <td class="tdBlue">
      <input name="groupId" id="groupId" maxlength="20" value="${group.groupId}" ></td>
  </tr>
  
  <tr>
    <td width="35%" class="tdBlue2">特征码组名称</td>
    <td class="tdBlue">
      <input name="groupName" id="groupName" maxlength="20" value="${group.groupName}" ></td>
  </tr>

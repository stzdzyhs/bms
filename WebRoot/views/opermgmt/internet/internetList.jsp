    ...
    <table class="tableCommon tableInterlace" width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <c:forEach items="${list}" var="u">
        <tr>
          ...
          <c:if test="${detail}"><a href='javascript:;' onclick='detail(${u.internetNo})'>详情</a></c:if>
          <c:if test="${edit && activeOperator.type !=2}"><a href="javascript:;" onclick="toEditPage('${u.internetNo }')">编辑</a></c:if>
          <c:if test="${region && u.parentId != -1}"> <a href="javascript:;" onclick="regionMgmt('${u.internetNo}')">区域管理</a></c:if>
          <c:if test="${delete}">
             <c:if test="${u.parentId != -1 && activeOperator.type !=2}">
                <a href="javascript:;" onclick="deleteSets('${u.internetNo }')">删除</a>
             </c:if>
         </c:if>
          </td>
        </tr>

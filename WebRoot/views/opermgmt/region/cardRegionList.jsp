...
$(document).ready(function(){
	...
	$("#regionSectionBegin").keyup(function(){
		...
		if (regionSectionBegin == null || regionSectionBegin == ''){
			if (regionSectionEnd == null || regionSectionEnd == ''){
				$("#btnQuery").removeAttr("disabled");
			}else{
	...
	$("#regionSectionEnd").keyup(function(){
		...
		else if (regionSectionEnd != null && regionSectionEnd != '' && !regionCodeReg.test(regionSectionEnd)){		
			$("#btnQuery").attr({"disabled":"disabled"});
		}else{
			$("#btnQuery").removeAttr("disabled");
		}
...
function deleteSets(id) {
	...
	art.dialog.confirm('你确认删除操作？', function(){
			var options = {
				url: "${path}/opermgmt/region/cardRegionDelete.action" + param,
				dataType: 'html',
				beforeSend: function() {
					art.dialog.through({
						id: 'broadcastLoading',
				...
				success: function(data) {
					...
					if(rsobj.result=="true"){
						...
						if (rsobj.desc=='reference'){
							art.dialog.alert("区域已经被策略引用，不能删除！");
						} else{
							art.dialog.alert("删除失败！");
						}
...
function goBack(){
	document.getElementById("form1").action = "${path}/opermgmt/region/cardRegionList.action";
	document.getElementById("form1").submit();
}
//进入新增编辑页面
function toEditPage(regionId){
	if(regionId!=null && typeof(regionId)!="undefined"){
		var options = {
				url: "${path}/opermgmt/region/checkCardRegionReference.action?regionIds="+regionId,
				dataType: 'html',
				beforeSend: function() {
					art.dialog.through({
						id: 'broadcastLoading',
				...
				success: function(data) {
					...
					if(rsobj.result=="true"){
						...
						document.getElementById("form1").action = "${path}/opermgmt/region/cardRegionEdit.action";
						document.getElementById("form1").submit();
					}else{
						art.dialog.alert("校验失败！");
					}
...
<form action="${path}/opermgmt/region/cardRegionList.action" method="post" name="form1" id="form1">
              ...
              <tbody>
                <tr>
                  ...
                  <td width="160">
      				...
      				onMouseOver="toolTip('名称由中文、大小写英文字母、数字、以及下划线组成')" onMouseOut="toolTip()" autocomplete="off" oncontextmenu="return false;"/>
      			  </td>
      ...
      <c:if test="${list == null || fn:length(list) == 0}">
        ...
        <tr>
          ...
          <td class="tdOpera2" align="center">
             <c:if test="${detail}"><a href="javascript:;" onclick="detail('${r.id }')">详情</a></c:if>
             <c:if test="${edit}"><a href="javascript:;" onclick="toEditPage('${r.id}')">编辑</a></c:if>
             <c:if test="${delete}"> <a href="javascript:;" onclick="deleteSets('${r.id}')">删除</a></c:if>
          </td>
        </tr>
        </c:forEach>
      </tbody>

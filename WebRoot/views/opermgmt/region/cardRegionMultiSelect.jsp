...
$(document).ready(function(){
	var selectRegionId = '${regionId}';
	var selectRegionIdArr = selectRegionId.split(',');
	if (selectRegionId != null && selectRegionId != ''){
		for (var i=0; i < selectRegionIdArr.length; i++){
			$("input[name='regionIds'][value=" + selectRegionIdArr[i] + "]").attr("checked","true");
		}
	}
});
function saveRegion(){
	var regionIds = $('input[name="regionIds"]:checked');
	...
	for (var i=0; i<regionIds.length; i++){
		var id = regionIds[i].value;
		var name = $("#regionName" + id).val();
		var code = "";
		var codeType = $("#codeType" + id).val();
		if (codeType == 0){
			code = $("#regionCode" + id).val();
			code = formatRegionCode(code);
		}else if (codeType == 1){
			var regionSectionBegin = $("#regionSectionBegin" + id).val();
			regionSectionBegin = formatRegionCode(regionSectionBegin);
			var regionSectionEnd = $("#regionSectionEnd" + id).val();
			regionSectionEnd = formatRegionCode(regionSectionEnd);
			code = regionSectionBegin + "-" + regionSectionEnd;
		}
	...
	regionName = regionName.substring(0,regionName.length - 1);
	 ...
	 artDialog.open.origin.document.getElementById('regionId').value = regionId;
...
<form action="${path}/opermgmt/region/cardRegionMultiSelect.action" method="post" name="form1" id="form1">
